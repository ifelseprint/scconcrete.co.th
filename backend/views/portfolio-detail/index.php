...
<div class="content-header">
  <div class="container">
    <div class="row mb-2">
      <div class="col-sm-6">
        <h1 class="m-0 text-dark"><i class="icofont icofont-coins"></i> Portfolio Detail <small>List</small></h1>
      </div><!-- /.col -->
      <div class="col-sm-6">
        <ol class="breadcrumb float-sm-right">
          <li class="breadcrumb-item"><a href="#"><i class="ti-home"></i> Home</a></li>
          <li class="breadcrumb-item active">Portfolio Detail List</li>
        </ol>
      </div><!-- /.col -->
    </div><!-- /.row -->
  </div><!-- /.container-fluid -->
    ...
    <div class="card">
      ...
      echo GridView::widget([
          ...
          'columns' => [
            ...
            [
                'attribute' => 'portfolio_detail_image',
                'label' => 'Image',
                'format' => 'raw',
                'contentOptions' => ['style' => 'width:90px'],
                'value' => function ($model) {
                    return '<img src="'.str_replace('/admin', '', Yii::getAlias('@web')).'/uploads/'.$model->portfolio_detail_image_path.'/'.$model->portfolio_detail_image.'" width="50">';
                },
            ],
            [
                ...
                'value' => function ($model) {
                  if(!empty($model->portfolio->portfolio_name_th)){
                    $portfolio = $model->portfolio->portfolio_name_th;
                  }else{
                    $portfolio = '-';
                  }
                  return '<div style="height: 25px;overflow:hidden;">'.$portfolio.'</div>';
                },
            ],
            [
                'attribute' => 'portfolio_detail_content_th',
                'label' => 'Portfolio Detail Name (TH)',
                'format' => 'raw',
                // 'contentOptions' => ['style' => 'width:120px'],
                'value' => function ($model) {
                    return '<div style="height: 25px;overflow:hidden;">'.$model->portfolio_detail_content_th.'</div>';
                },
            ],
            [
                'attribute' => 'portfolio_detail_content_en',
                'label' => 'Portfolio Detail Name (EN)',
                'format' => 'raw',
                // 'contentOptions' => ['style' => 'width:120px'],
                'value' => function ($model) {
                    return '<div style="height: 25px;overflow:hidden;">'.$model->portfolio_detail_content_en.'</div>';
                },
            ],
            [
              ...
              'buttons'  => [
                'Update' => function ($url,$model) {
                  return Html::a('<span class="fa fa-edit" style="padding:0px 2px;"></span>','', [
                    ...
                    'title' => 'edit',
                    'data-toggle' => 'modal',
                    'value' => Url::to('portfolio-detail/update/'.$model->portfolio_detail_id),
                  ]);
                },
                'Delete' => function ($url, $model) {
                  return Html::a('<span class="fas fa-trash-alt" style="padding:0px 2px;"></span>', Url::to('portfolio-detail/delete/'.$model->portfolio_detail_id), [
                    'title' => 'delete',
                    'data-confirm' => Yii::t('yii', 'Are you sure you want to delete '.$model->portfolio_detail_id.' this item?'),
                    'data-method'  => 'post',
                    'data-pjax' => 1
                  ]);
                },
